...
const OWNER_BLUE = {
  primary: '#0B1F3A',
  primaryPressed: '#08182D',
  soft: '#EFF6FF',
  softBorder: '#BFDBFE',
  onPrimary: '#FFFFFF',
} as const;

export type OwnerWizardPhase = 'coordination' | MeetupLifecyclePhase;

export type OwnerWizardShellProps = {
  phase: OwnerWizardPhase;
...
function themeForPhase(phase: OwnerWizardPhase): ShellTheme {
  if (phase === 'coordination') return OWNER_BLUE;
  return MEETUP_LIFECYCLE_THEME[phase];
}

export function OwnerWizardShell({
  phase,
  progressIndex,
  title,
  subtitle,
  ...
  const showLifecycleHeader = phase !== 'coordination' && progressIndex != null;
  ...
  return (
    ...
    >
      ...
      >
        {showLifecycleHeader ? (
          <MeetupLifecycleProgressHeader
            activePhase={phase as MeetupLifecyclePhase}
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + 12 }]}>
        ...
        <Pressable
          ...
          style={({ pressed }) => [
            ...
            pressed && { backgroundColor: theme.primaryPressed ?? theme.primary },
            (primaryDisabled || primaryBusy) && styles.btnDisabled,
          ]}
        >
          {primaryBusy ? (
            <ActivityIndicator color={theme.onPrimary ?? '#FFFFFF'} />
          ) : (
            <Text style={[styles.primaryBtnText, { color: theme.onPrimary ?? '#FFFFFF' }]}>
              {primaryLabel}
            </Text>
          )}
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  secondaryBtnText: { fontSize: 15, fontWeight: '600', color: '#0F172A' },
