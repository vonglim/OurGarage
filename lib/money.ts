...
export function getNumericOfferPrice(offer: { price?: unknown; currentPrice?: unknown }): number {
  if (typeof offer.currentPrice === 'number' && Number.isFinite(offer.currentPrice)) {
    return offer.currentPrice;
  }
  if (typeof offer.price === 'number' && Number.isFinite(offer.price)) {
    return offer.price;
  }
  return parseMoneyToNumber(String(offer.currentPrice ?? offer.price ?? '')) ?? 0;
}
